...
**v3_9** (현재): `master.sty v1_24` 변경 사항 반영. LuaTeX-ja 줄바꿈 규칙 추가. `\citework` 조사 처리 규칙 추가. tikz 도식 작업흐름 명시. `nobottomtitles*` 적용으로 헤딩·본문 분리 문제 해결.
...
KoPub 바탕체(한국어)와 Brill(라틴/그리스) 혼용 환경에서는 **줄바꿈 위치**에 따라 공백이 소실되는 현상이 발생한다. `master.sty v1_24`의 `xkanjiskip` 전역 설정으로 대부분 해결되나, 아래 두 패턴은 여전히 주의가 필요하다.
