...
**v3_10** (현재): `master.sty v1_25` 전면 반영. LuaTeX-ja 줄바꿈 규칙 신설. `\citework` 조사 처리 규칙 신설. 도식·도표 섹션 전면 개정. `\concepttable` 환경 규칙 추가. `nobottomtitles*` + `\needspace` 병용 원칙 추가.
...
| 상황 | 방식 |
|------|------|
| 원문 1–2줄, 본문 흐름에 녹아드는 경우 | 인라인 |
| 원문을 단독 강조하고 싶은 경우 | `\srcquote{원문}{번역}{출처}` |
| 원문 또는 번역이 3줄 이상 | `\bilingualquote{원문}{번역}{출처}` |

`\bilingualquote` 번역란 각주는 반드시 `\bqnote{}`를 사용한다.
...
**유형 1 — 분류 격자**: 두 기준 교차로 4칸 이상 산출. *해당: 1주차 사중 분류, 4주차 대립 유형.*

**유형 2 — 위계 나무**: 유→종→개별자 포함 관계. *해당: 1주차 포르피리오스의 나무, 2주차 제1·제2실체.*

**유형 3 — 타임라인**: 연대순 지적 계보. *해당: 5주차 수용사.*

**유형 4 — 개념 대조표**: 같은 기준으로 비교하는 항목 4개 이상. *해당: 3주차 질의 네 종류, 4주차 대립 유형.*

**유형 5 — 방사 구조**: 한 항목에 모든 것이 의존. *해당: 2주차 제1실체 우선성.*
