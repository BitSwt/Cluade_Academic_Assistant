...
**모든 문서 작성·파일 변환·빌드·검수는 이 레포에서 클론해 온 스크립트와 규칙 문서를 엄격히 따른다.** 세션 중에 즉석으로 새 스크립트를 작성하거나, 레포에 있는 스크립트의 로직을 임의로 재구현하지 않는다. 레포에 이미 존재하는 도구가 있으면 반드시 그것을 사용한다. 레포의 스크립트나 규칙에 버그나 부족한 점이 발견되면, 즉석 우회가 아니라 해당 파일 자체를 수정하여 레포에 반영한다.
